...
class SceneManager:
    ...
    def _prepare_new_game(self, cast, script):
        ...
        self._add_lives(cast, 'left')
        self._add_lives(cast, 'right')

        cast.clear_actors(RACKET_GROUP)
        self._add_racket(cast, 'left')
        self._add_racket(cast, 'right')
    ...
    def _prepare_try_again(self, cast, script):
        ...
        stats.reset_score()        # when try again the score becomes 0
        self._add_ball(cast)        
        self._add_dialog(cast, PREP_TO_LAUNCH)
        script.clear_actions(INPUT)
        script.add_action(INPUT, TimedChangeSceneAction(IN_PLAY, 2))
    ...
    def _prepare_game_over(self, cast, script):
        self._add_ball(cast)
        # verify which player won
        racket_left = cast.get_first_actor(RACKET_GROUP)
        racket_right = cast.get_second_actor(RACKET_GROUP)
        if racket_left.get_lives() == 0:
            self._add_dialog(cast, PLAYER2_WON)         # player 2 won because player 1 has no lives
        elif racket_right.get_lives() == 0:
            self._add_dialog(cast, PLAYER1_WON)         # player 1 won because player 2 has no lives
        else:
            self._add_dialog(cast, WAS_GOOD_GAME)       # the game is over for other reason
    ...
    def _add_level(self, cast):
        ...
        text = Text(LEVEL_FORMAT, FONT_FILE, FONT_SMALL, ALIGN_CENTER)
        position = Point(CENTER_X + 100, HUD_MARGIN)
        label = Label(text, position)
        cast.add_actor(LEVEL_GROUP, label)

    def _add_lives(self, cast, side):
        if side == 'left':
    ...
    def _add_racket(self, cast, side):
        if side == 'left':
            x = int(CENTER_X / 3) - RACKET_WIDTH / 2
            image = Image(RACKET_IMAGE_LEFT)
        else:
            x = int(CENTER_X / 3 * 5) - RACKET_WIDTH / 2
            image = Image(RACKET_IMAGE_RIGHT)
